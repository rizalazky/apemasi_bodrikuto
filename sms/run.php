...
$hasil = mysql_query("SELECT * FROM inbox WHERE Processed = 'false'");
  ...
  //command daftar
	elseif ($command=="DAFTAR"){
		  // cek jumlah parameter test
		    if (count($split) >= 2)
		      {
		        $nama=$split[1];
				$kode_bendung=$split[2];
			       // cek apakah nomor hp terdaftar
			       if (ceknohp($nohp) == 1)
			       {
             ...
             //input ke database
                  ...
                  mysql_query("INSERT INTO phonebook(nama, no_hp,kode_bendung,password) VALUES  ('$nama','$nohp','$kode_bendung','pemalicomal')");
                  //revisi 8 januari 2021 registrasi via sms ke hosting 
				  $auth = md5("pemalicomal");                  
				  $curldaftar = curl_init();
				  curl_setopt($curldaftar, CURLOPT_URL,$web."/register.php");
				  curl_setopt($curldaftar, CURLOPT_POST, 1);
				  curl_setopt($curldaftar, CURLOPT_POSTFIELDS, "nama=" .$nama.
													"&no_hp=" .$nohp. 
													"&kode_bendung=" .$kode_bendung.
													"&auth=" .$auth. 
													"&sms=" .$sms.
													"&nohp=" .$nohp.
													"&time=" .$time
													);
				  curl_exec ($curldaftar);
				  curl_close ($curldaftar);
				  //end revisi 8 januari 2021 registrasi via sms ke hosting  
				  }
		      }
